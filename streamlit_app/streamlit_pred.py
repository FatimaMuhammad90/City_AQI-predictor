...
def get_latest_predictions(city):
    response = supabase.table("predictions").select("*").eq("city", city).order("prediction_time", desc=True).limit(1).execute()
    ...
    response = supabase.table("predictions").select("*").eq("city", city).eq("prediction_time", latest_time).execute()
...
def get_hourly_backfill_status():
    ...
    evaluated = sum(1 for prediction in response.data if prediction["evaluated"])
