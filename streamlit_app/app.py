...
if st.session_state["predictions"] is not None:
    ...
    chart_data = pd.DataFrame(
        {
            "Forecast Horizon": ["24 Hours", "48 Hours", "72 Hours"],
            "Predicted AQI": [aqi_24, aqi_48, aqi_72]
        }
    )

    st.line_chart(chart_data.set_index("Forecast Horizon"), use_container_width=True)
...
if not monitoring_data:
    ...
    for _, row in latest.iterrows():
        with st.expander(f"{row['city']} - {row['horizon']}h ({row['model']})"):
            ...
            # ------------------------------------------------
            historical_predictions = get_predictions_with_actuals(row["city"], row["horizon"])
            ...
            if historical_predictions:
                ...
                chart_df = pred_df.set_index("target_time")[["predicted_aqi", "actual_aqi", "lagged_aqi"]]

                st.line_chart(chart_df, height=300)

                st.caption("Predicted AQI vs Actual AQI vs Previous-hour AQI")
