...
def store_historical_data(rows):
    ...
    try:
        (
            ...
            .upsert(rows, on_conflict="city,timestamp_utc")
            .execute()
        )
...
def backfill_predictions():
    ...
    for city, predictions in city_groups.items():
        ...
        try:
            ...
            weather_lookup = weather_df.set_index("timestamp_utc").to_dict(
                "index"
            )
            ...
            for prediction in predictions:
                ...
                try:
                    ...
                    print(
                        ...
                        f"pred={predicted_aqi:.2f}, actual={actual_aqi:.2f}, error={error}%"
                    )
                except Exception as e:
                    print(
                        f"  WARNING: Could not update prediction {prediction_id}: {e}"
                    )
                ...
                historical_rows.append({
                    ...
                    "pm2_5": float(actual_data["pm2_5"])
                    if pd.notna(actual_data["pm2_5"])
                    else None,
                    "pm10": float(actual_data["pm10"])
                    if pd.notna(actual_data["pm10"])
                    else None,
                    "co": float(actual_data["co"])
                    if pd.notna(actual_data["co"])
                    else None,
                    "no2": float(actual_data["no2"])
                    if pd.notna(actual_data["no2"])
                    else None,
                    "so2": float(actual_data["so2"])
                    if pd.notna(actual_data["so2"])
                    else None,
                    "o3": float(actual_data["o3"])
...
def delete_previous_week_predictions(start_date, end_date):
    ...
    print( f"\nDeleting predictions from " f"{start_date} to {end_date}...")
    ...
    response = (
        ...
        .delete()
        .eq("evaluated", True)
        .gte("target_time", start_str)
        .lt("target_time", end_str)
        .execute()
    )
    ...
    print(
        f" Deleted {len(deleted)} previous-week predictions."
    )
...
def calculate_mae(predictions):
    ...
    df = pd.DataFrame(predictions).dropna(
        subset=["predicted_aqi", "actual_aqi"]
    )
    if df.empty:
        return None
...
def get_daily_status(predictions):
    ...
    for day, group in df.groupby("date"):
        # Day is bad if prediction error exceeds threshold
        daily[day] = group["error_percent"].gt(ERROR_THRESHOLD).any()
...
def get_max_consecutive_bad_days(daily_status):
    ...
    for current_date in dates:
        if daily_status[current_date] and (
...
def run_weekly_monitoring():
        ...
        for horizon in horizons:
            ...
            try:
                predictions = get_week_predictions(
                    city, horizon, start_date, end_date
                )
                if not predictions:
                ...
                flag_bad_days = consecutive_bad >= CONSECUTIVE_BAD_DAYS
                flag_mae = mae is not None and mae > baseline * MAE_MARGIN
                flagged = flag_bad_days or flag_mae
    ...
    # deletion of older prediction
    if results:

        print(
            f"\n--- Inserting {len(results)} "
            f"records into monitoring table ---"
        )
        response = ( supabase.table("monitoring").insert(results).execute())
        if response.data:
            print("Monitoring records saved.")
            try:
                delete_previous_week_predictions(start_date,end_date)
            except Exception as e:
                print(f"Monitoring was saved, but prediction cleanup failed: {e}")
        else:
            print("Monitoring insertion returned no data.")
